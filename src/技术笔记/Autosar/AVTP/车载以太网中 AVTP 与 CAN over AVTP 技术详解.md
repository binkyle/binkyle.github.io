---
title: "AVTP 与 CAN over AVTP 在车载以太网中的协议详解与工程实践"
shortTitle: "AVTP & CAN over AVTP"
icon: network
date: 2025-06-18
order: 1
isOriginal: true
...
```mermaid
sequenceDiagram
    participant ECU_A_CAN as ECU A\nCAN接口 (CAN0)
    participant ECU_A_1722 as ECU A\n1722Tp模块
    participant ECU_A_Eth as ECU A\n以太网控制器
    participant ECU_B_Eth as ECU B\n以太网控制器
    participant ECU_B_1722 as ECU B\n1722Tp模块
    participant ECU_B_CAN as ECU B\nCAN接口 (CAN1)

    ECU_A_CAN->>ECU_A_1722: 接收到CAN帧后交给1722Tp (PduR 路由)
    note over ECU_A_1722: 1722Tp将CAN帧封装为ACF消息<br/>加入AVTP帧 (可合并多帧)
    ECU_A_1722->>ECU_A_Eth: 发送AVTP帧 (EtherType=0x22F0)
    ECU_A_Eth-->>ECU_B_Eth: 以太网交换传输AVTP帧
    ECU_B_Eth->>ECU_B_1722: 收到1722帧，上报1722Tp处理
    note over ECU_B_1722: 1722Tp解析AVTP帧头，提取出<br/>封装的CAN帧 (ACF报文解包)
    ECU_B_1722->>ECU_B_CAN: 将CAN帧交付目标CAN接口 (CanIf)
    note over ECU_B_CAN: CAN接口将帧发送到本地CAN总线<br/>或交给上层应用处理
```
